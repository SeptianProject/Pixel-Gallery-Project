import { useLocation, useNavigate } from "react-router-dom";
import { assets } from "../../assets/assets";
import { BounceInTop } from "../animations/BounceAnimate";

const CardProject = ({ project }) => {
  const navigate = useNavigate();
  const profile = project?.profiles;

  return (
    <BounceInTop>
      <div
        onClick={() =>
          navigate(`/project/detail/${project.slug}?p=${project.id}`)
        }
        className="flex flex-col pt-12 select-none cursor-pointer"
      >
        <div className="relative group rounded-2xl hover:shadow-bottom-dark hover:border-transparent hover:border-b transition-all duration-300">
          <img
            src={project.image_cover ? project.image_cover : assets.card_image}
            className="aspect-video rounded-xl"
          />
          <div className="absolute rounded-b-2xl bottom-0 right-0 from-dark bg-gradient-to-t w-full h-1/2 to-transparent transition-all duration-300 opacity-0 group-hover:opacity-100"></div>
          <div className="opacity-0 group-hover:opacity-100 flex absolute bottom-0 h-10">
            <h1 className="pl-6 text-white">{project?.title}</h1>
          </div>
        </div>
      </div>
      <div
        onClick={() => navigate(`/dashboard/${profile.id}/${profile.name}`)}
        className="flex items-center mt-4 gap-4 cursor-pointer"
      >
        <img
          src={profile?.avatar_url ? profile.avatar_url : assets.photo_profile}
          className="h-8 w-auto hidden lg:block rounded-full"
        />
        <h4 className="text-dark font-semibold text-base">{profile?.name}</h4>
      </div>
    </BounceInTop>
  );
};

export default CardProject;
